@edit_apartment
Feature: API should edit apartment by id

  Background:
    Given Addresses with ids:
      """
      4c2ec2f3-ddba-22cb-7f55-a76ac154ad1f
      c4d6c866-7130-7da9-6646-41b5cd4d5e20
      d91b0d9a-0083-f483-aa12-d81ba8aa339f
      244a4ed1-85b2-9dc4-2926-11c886de6a80
      7cddf877-ea6e-8c60-3b05-3ccf7cc4c105
      bedb4a88-4883-20a6-00c6-cd79dd7ab1a0
      49fa067f-c9e3-cd70-6737-e60ead448914
      9507cd3a-c9c6-c11f-2bc0-f5ce74a12d67
      """

  Scenario: Edit apartment with rooms
    Given apartment with following config:
    """
    {
      "apartment": {
          "id": 1,
          "exposed_id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
          "square": "75",
          "floor": "1",
          "built_in": "2009-06-15",
          "user_id": "123",
          "price": "1231232",
          "apartment_type": "RegularApartment",
          "heating_type": "HeatPump",
          "rental_price": "1111",
          "currency": "USD",
          "has_gas": true,
          "has_water": true,
          "has_hood": true,
          "title": "title",
          "description": "description"
      },
      "rooms": [
          {
              "room_type": "LivingRoom",
              "square": 15,
              "exposed_id": "244a4ed1-85b2-9dc4-2926-11c886de6a80"
          },
          {
              "room_type": "Bedroom",
              "square": 25,
              "exposed_id": "39a26a59-310f-d5e6-1c3f-1bd5f9816f9b"
          },
          {
              "room_type": "Kitchen",
              "square": 35,
              "exposed_id": "63b8885e-54c3-80db-1952-a0f74e0caf35"
          }
      ],
      "addresses": [
          "c4d6c866-7130-7da9-6646-41b5cd4d5e20",
          "d91b0d9a-0083-f483-aa12-d81ba8aa339f",
          "244a4ed1-85b2-9dc4-2926-11c886de6a80"
      ]
    }
    """
    And apartment will have the following config:
    """
    {
      "apartment": {
          "id": 1,
          "exposed_id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
          "square": "78",
          "floor": "2",
          "built_in": "2010-06-15",
          "user_id": "123",
          "price": "5555555",
          "apartment_type": "AlcoveStudio",
          "heating_type": "Furnace",
          "rental_price": "1111",
          "currency": "USD",
          "has_gas": false,
          "has_water": false,
          "has_hood": false,
          "title": "title123",
          "description": "descriptionabsdefg"
      },
      "rooms": [
          {
              "room_type": "LivingRoom",
              "square": 15,
              "exposed_id": "244a4ed1-85b2-9dc4-2926-11c886de6a80"
          },
          {
              "room_type": "GameRoom",
              "square": 21,
              "exposed_id": "39a26a59-310f-d5e6-1c3f-1bd5f9816f9b"
          },
          {
              "room_type": "Pantry",
              "square": 35,
              "exposed_id": "63b8885e-54c3-80db-1952-a0f74e0caf35"
          }
      ]
    }
    """
    When I try to edit this apartment
    When I ask for apartment with id "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5"
    Then I ensure that all apartment values were changed:
    """
    {
      "id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
      "square": 78,
      "floor": 2,
      "builtIn": "2010-06-15",
      "rooms": [
          {
            "id": "244a4ed1-85b2-9dc4-2926-11c886de6a80",
            "roomType": "LivingRoom",
            "square": 15
          },
          {
            "id": "39a26a59-310f-d5e6-1c3f-1bd5f9816f9b",
            "roomType": "GameRoom",
            "square": 21
          },
          {
            "id": "63b8885e-54c3-80db-1952-a0f74e0caf35",
            "roomType": "Pantry",
            "square": 35
          }
      ],
      "ownerId": 123,
      "price": 5555555,
      "apartmentType": "AlcoveStudio",
      "heatingType": "Furnace",
      "rentalPrice": 1111,
      "currency": "USD",
      "hasGas": false,
      "hasWater": false,
      "hasHood": false,
      "title": "title123",
      "description": "descriptionabsdefg",
      "addresses": [
        [
          {
            "id": "c4d6c866-7130-7da9-6646-41b5cd4d5e20"
          },
          {
            "id": "d91b0d9a-0083-f483-aa12-d81ba8aa339f"
          },
          {
            "id": "244a4ed1-85b2-9dc4-2926-11c886de6a80"
          }
        ]
      ]
    }
    """

  Scenario: Edit apartment without rooms
    Given apartment with following config:
    """
    {
      "apartment": {
          "id": 1,
          "exposed_id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
          "square": "75",
          "floor": "1",
          "built_in": "2009-06-15",
          "user_id": "123",
          "price": "1231232",
          "apartment_type": "RegularApartment",
          "heating_type": "HeatPump",
          "rental_price": "1111",
          "currency": "USD",
          "has_gas": true,
          "has_water": true,
          "has_hood": true,
          "title": "title",
          "description": "description"
      },
      "rooms": [],
      "addresses": [
          "c4d6c866-7130-7da9-6646-41b5cd4d5e20",
          "d91b0d9a-0083-f483-aa12-d81ba8aa339f",
          "244a4ed1-85b2-9dc4-2926-11c886de6a80"
      ]
    }
    """
    And apartment will have the following config:
    """
    {
      "apartment": {
          "id": 1,
          "exposed_id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
          "square": "78",
          "floor": "2",
          "built_in": "2010-06-15",
          "user_id": "123",
          "price": "5555555",
          "apartment_type": "AlcoveStudio",
          "heating_type": "Furnace",
          "rental_price": "1111",
          "currency": "USD",
          "has_gas": false,
          "has_water": false,
          "has_hood": false,
          "title": "title123",
          "description": "descriptionabsdefg"
      },
      "rooms": []
    }
    """
    When I try to edit this apartment
    When I ask for apartment with id "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5"
    Then I ensure that all apartment values were changed:
    """
    {
      "id": "e4388052-2fc9-9d1e-b9f0-077b17eb0cd5",
      "square": 78,
      "floor": 2,
      "builtIn": "2010-06-15",
      "rooms": [],
      "ownerId": 123,
      "price": 5555555,
      "apartmentType": "AlcoveStudio",
      "heatingType": "Furnace",
      "rentalPrice": 1111,
      "currency": "USD",
      "hasGas": false,
      "hasWater": false,
      "hasHood": false,
      "title": "title123",
      "description": "descriptionabsdefg",
      "addresses": [
        [
          {
            "id": "c4d6c866-7130-7da9-6646-41b5cd4d5e20"
          },
          {
            "id": "d91b0d9a-0083-f483-aa12-d81ba8aa339f"
          },
          {
            "id": "244a4ed1-85b2-9dc4-2926-11c886de6a80"
          }
        ]
      ]
    }
    """